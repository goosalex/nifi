...
@CapabilityDescription("Consumes Avro messages from Apache Kafka specifically built against the Kafka 2.0 Consumer API and encoded using the Confluent or Hortonworks Schema Registry."
    ...
    + "The complementary NiFi processor for sending messages is PublishKafkaRecord_2_6, configured with AvroRecordSetWriter using a compatible Schema Registry.")
...
public class ConsumeKafkaAvro_2_6 extends AbstractProcessor {
    ...
    static final PropertyDescriptor SCHEMA_SERVICE = new PropertyDescriptor.Builder()
            .name("schema-registry-service")
            .displayName("Schema Registry")
            .description("The Schema Registry used to decode the avro messages and attribute a Schema.")
            .identifiesControllerService(SchemaRegistry.class)
            .expressionLanguageSupported(ExpressionLanguageScope.NONE)
            .addValidator((subject, input, validationContext) -> {
                return new ValidationResult.Builder().valid(validationContext.getControllerServiceLookup().getControllerService(input).getClass().getCanonicalName().endsWith("ConfluentSchemaRegistry")).build();
            })
            .required(true)
            .build();
    ...
    static {
        ...
        descriptors.add(SCHEMA_SERVICE);
        descriptors.add(AVRO_CODEC);
    ...
    protected ConsumerPool createConsumerPool(final ProcessContext context, final ComponentLog log) {
        ...
        final String topicListing = context.getProperty(ConsumeKafkaAvro_2_6.TOPICS).evaluateAttributeExpressions().getValue();
        final String topicType = context.getProperty(ConsumeKafkaAvro_2_6.TOPIC_TYPE).evaluateAttributeExpressions().getValue();
        ...
        if (topicType.equals(TOPIC_NAME.getValue())) {
            ...
            return new ConsumerPool(maxLeases, schemaAccessStrategy,codecFactory, props, topics, maxUncommittedTime, maxPollRecords, keyEncoding, securityProtocol,
            ...
            return new ConsumerPool(maxLeases, schemaAccessStrategy,codecFactory, props, topicPattern, maxUncommittedTime, maxPollRecords, keyEncoding, securityProtocol,
                bootstrapServers, log, honorTransactions, charset, headerNamePattern);
        } else {
